...
def generate_detailed_analysis(analysis_df, overall_stability, overall_mttr, overall_mtbf, analysis_level):
    ...
    if len(analysis_df) > 1:
        ...
        y = analysis_df['stability'].fillna(analysis_df['stability'].mean()) # Handle potential NaNs
        slope, _ = np.polyfit(x, y, 1)
        
        if slope > 2: trend_direction = "a clear upward (improving) trend"
        elif slope < -2: trend_direction = "a clear downward (declining) trend"
        else: trend_direction = "a stable but fluctuating trend"
        
        predictive_insight = f"Over the period, the performance has been <strong>{volatility_level}</strong> and shows <strong>{trend_direction}</strong>."
    ...
    # --- 4. Detailed Period-by-Period Breakdown ---
    period_breakdown = []
    if len(analysis_df) <= 12: # Only show for a reasonable number of periods
        for _, row in analysis_df.iterrows():
            period_label = format_period(row['period'], analysis_level)
            s_class = "excellent" if row['stability'] > 90 else "good" if row['stability'] > 70 else "poor"
            stops = int(row['stops'])
            mttr = row.get('mttr', 0)
            period_breakdown.append(f"<li><strong>{period_label}:</strong> Had <strong>{s_class} stability</strong> ({row['stability']:.1f}%) with {stops} stops (avg. {mttr:.1f} min downtime).</li>")
    ...
    return {
        ...
        "breakdown": "".join(period_breakdown),
    ...
    if st.session_state.show_llm_analysis:
        ...
        if "error" in insights:
            ...
            if insights['breakdown']:
                ...
                <ul style="padding-left: 20px; margin-top: 5px;">{insights['breakdown']}</ul>
                """
            analysis_html += f"""
