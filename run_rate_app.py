...
class RunRateCalculator:
    ...
    def _calculate_all_metrics(self) -> dict:
        ...
        if df.empty or "ACTUAL CT" not in df.columns:
             # Check if only time columns exist, suitable for export but not full analysis
            if "time_diff_sec" in df.columns:
                 # Allow processing for export even without ACTUAL CT, but metrics will be limited
                pass
            else:
        ...
        if "ACTUAL CT" in df.columns:
            if self.analysis_mode == 'by_run' and 'run_id' in df.columns:
             ...
             # If no ACTUAL CT, limits cannot be calculated based on it
             lower_limit = 0 # Define explicitly
             upper_limit = np.inf


        # --- Stop Detection Logic ---
        # Initialize flags assuming ACTUAL CT might be missing
        df["stop_flag"] = 0
        df["stop_event"] = False
        ...
        if "ACTUAL CT" in df.columns:
            ...
            # Use fillna(0) for prev_actual_ct to handle the first row comparison safely
            ...
            df["stop_flag"] = np.where(is_abnormal_cycle | is_downtime_gap.fillna(False), 1, 0)
             ...
             # If no ACTUAL CT, only downtime gap can trigger a stop (conceptual)
             # However, without prev_actual_ct, the gap tolerance logic isn't fully applicable.
             # A simple large time diff might indicate a stop, but this needs definition.
             # For now, assume no stops if ACTUAL CT is missing, focus on export.
             df["stop_flag"] = 0 # Default to no stops
        ...
        # Run durations based on ACTUAL CT (if available) or adj_ct_sec
        run_duration_col = "ACTUAL CT" if "ACTUAL CT" in df.columns else "adj_ct_sec"
        # Filter potential large stop gaps used for run splitting
        df_for_runs = df[df['adj_ct_sec'] <= (24 * 3600)].copy() # Use a reasonable upper limit like 24h
        ...
        # Use 'adj_ct_sec' for the time *before* the first stop
        ...
        # Wall clock time for report header
        ...
        if not run_durations.empty:
            max_minutes = min(run_durations["duration_min"].max(), 240) # Cap at 4 hours for buckets
        ...
        final_results = {
            ...
            # --- Metrics for Exporter Compatibility ---
            "avg_cycle_time_sec": avg_cycle_time_sec,
            "time_to_first_dt_min": time_to_first_dt_sec / 60,
            "production_run_sec": production_run_sec, # Wall-clock total time
            "tot_down_time_sec": downtime_sec # Alias for exporter
        }
        ...
        elif "ACTUAL CT" in df.columns : # Only add these if ACTUAl CT exists
...
def plot_shot_bar_chart(df, lower_limit, upper_limit, mode_ct, time_agg='hourly'):
    if df.empty or 'adj_ct_sec' not in df.columns: # Check needed column exists
        st.info("No shot data with cycle times to display."); return
    df = df.copy()
    ...
    # Handle cases where limits might not be calculated (e.g., no ACTUAL CT)
    ...
    elif ll is not None and ul is not None and ul != np.inf: # Only draw aggregate band if limits are valid
        if not df.empty:
            fig.add_shape(type="rect", xref="x", yref="y", x0=df['shot_time'].min(), y0=ll, x1=df['shot_time'].max(), y1=ul, fillcolor=PASTEL_COLORS['green'], opacity=0.3, layer="below", line_width=0)
    ...
    if isinstance(mode_ct, (int, float)) and mode_ct > 0:
         ...
         mean_mode = df['mode_ct'].mean()
         if pd.notna(mean_mode) and mean_mode > 0:
              y_axis_cap_val = mean_mode
...
def plot_mttr_mtbf_chart(df, x_col, mttr_col, mtbf_col, shots_col, title):
    if df is None or df.empty or shots_col not in df.columns or df[shots_col].sum() == 0:
        st.info(f"Not enough data to plot {title}.")
        return
...
def format_minutes_to_dhm(total_minutes):
    ...
    days = total_minutes // (60 * 24); remaining_minutes = total_minutes % (60 * 24)
    hours = remaining_minutes // 60; minutes = remaining_minutes % 60
    parts = []
...
def calculate_daily_summaries_for_week(df_week, tolerance, downtime_gap_tolerance, analysis_mode):
    ...
    for date in sorted(df_week['date'].unique()):
        ...
        if not df_day.empty:
            ...
            res = calc.results
            summary = {'date': date, 'stability_index': res.get('stability_index', np.nan), 'mttr_min': res.get('mttr_min', np.nan), 'mtbf_min': res.get('mtbf_min', np.nan), 'stops': res.get('stop_events', 0), 'total_shots': res.get('total_shots', 0)}
...
def calculate_weekly_summaries_for_month(df_month, tolerance, downtime_gap_tolerance, analysis_mode):
    ...
    for week in sorted(df_month['week'].unique()):
        ...
        if not df_week.empty:
            ...
            res = calc.results
            summary = {'week': week, 'stability_index': res.get('stability_index', np.nan), 'mttr_min': res.get('mttr_min', np.nan), 'mtbf_min': res.get('mtbf_min', np.nan), 'stops': res.get('stop_events', 0), 'total_shots': res.get('total_shots', 0)}
...
def calculate_run_summaries(df_period, tolerance, downtime_gap_tolerance):
    ...
    for run_label, df_run in df_period.groupby('run_label'):
        if not df_run.empty:
            calc = RunRateCalculator(df_run.copy(), tolerance, downtime_gap_tolerance, analysis_mode='aggregate') # Always aggregate for individual run summary
            res = calc.results
            summary = {
                'run_label': run_label, 'start_time': df_run['shot_time'].min(), 'end_time': df_run['shot_time'].max(),
                'total_shots': res.get('total_shots', 0), 'normal_shots': res.get('normal_shots', 0),
                'stopped_shots': res.get('total_shots', 0) - res.get('normal_shots', 0),
                'mode_ct': res.get('mode_ct', 0), 'lower_limit': res.get('lower_limit', 0), 'upper_limit': res.get('upper_limit', 0),
                'total_runtime_sec': res.get('total_runtime_sec', 0), 'production_time_sec': res.get('production_time_sec', 0),
                'downtime_sec': res.get('downtime_sec', 0), 'mttr_min': res.get('mttr_min', np.nan),
                'mtbf_min': res.get('mtbf_min', np.nan), 'stability_index': res.get('stability_index', np.nan),
...
# --- Analysis Engine Functions (Unchanged) ---
def generate_detailed_analysis(analysis_df, overall_stability, overall_mttr, overall_mtbf, analysis_level):
    if analysis_df is None or analysis_df.empty: return {"error": "Not enough data for trend analysis."}
    ...
    overall_summary = f"Overall stability is <strong>{overall_stability:.1f}%</strong> ({stability_class})."
    predictive_insight = ""
    if len(analysis_df) > 1:
        ...
        predictive_insight = f"Performance shows a <strong>{trend_direction} trend</strong> and has been <strong>{volatility_level}</strong>."
    best_worst_analysis = ""
    if not analysis_df.empty:
        ...
        best_worst_analysis = (f"Best performance: <strong>{best_p}</strong> (Stability: {best_performer['stability']:.1f}%, Stops: {int(best_performer['stops'])}, MTTR: {best_performer.get('mttr', 0):.1f} min). Worst: <strong>{worst_p}</strong> (Stability: {worst_performer['stability']:.1f}%, Stops: {int(worst_performer['stops'])}, MTTR: {worst_performer.get('mttr', 0):.1f} min).")
    pattern_insight = ""
    if not analysis_df.empty and analysis_df['stops'].sum() > 0:
        if "Daily" in analysis_level:
            ...
            pattern_insight = f"Stop events peak around <strong>{int(peak_stop_hour['period'])}:00</strong> ({int(peak_stop_hour['stops'])} stops)."
        else:
            ...
            if not outliers.empty:
                ...
                pattern_insight = f"<strong>{outlier_label}</strong> performed significantly below average, impacting overall stability."
    recommendation = ""
    if overall_stability >= 95: recommendation = "Excellent performance. Monitor MTBF/MTTR trends."
    elif overall_stability > 70:
        recommendation = f"Good performance. Focus on {'<strong>MTBF</strong>' if (overall_mtbf > 0 and overall_mttr > 0 and overall_mtbf < overall_mttr * 5) else '<strong>MTTR</strong>'}. Current MTBF: {overall_mtbf:.1f} min, MTTR: {overall_mttr:.1f} min."
    else:
        recommendation = f"Poor stability. Primary driver: {'Low <strong>MTBF</strong>' if (overall_mtbf > 0 and overall_mttr > 0 and overall_mtbf < overall_mttr) else 'High <strong>MTTR</strong>'}. Current MTBF: {overall_mtbf:.1f} min, MTTR: {overall_mttr:.1f} min. Investigate root cause."
...
def generate_bucket_analysis(complete_runs, bucket_labels):
    ...
    try: long_buckets = [l for l in bucket_labels if int(l.split('-')[0].replace('+', '')) >= 60]
    except: long_buckets = []
    num_long_runs = complete_runs[complete_runs['time_bucket'].isin(long_buckets)].shape[0] if long_buckets else 0
    pct_long = (num_long_runs / total_runs * 100) if total_runs > 0 else 0
    longest_run = format_minutes_to_dhm(complete_runs['duration_min'].max())
...
def generate_mttr_mtbf_analysis(analysis_df, analysis_level):
    if analysis_df is None or analysis_df.empty or 'stops' not in analysis_df or analysis_df['stops'].sum() == 0 or len(analysis_df) < 2 or 'stability' not in analysis_df or 'mttr' not in analysis_df: return "Not enough data for correlation analysis."
    ...
    if not pd.isna(stops_corr) and not pd.isna(mttr_corr):
        if abs(stops_corr) > abs(mttr_corr) * 1.5: primary_driver = "the **frequency of stops**"; primary_driver_freq = True
        elif abs(mttr_corr) > abs(stops_corr) * 1.5: primary_driver = "the **duration of stops**"; primary_driver_dur = True
        else: primary_driver = "both the **frequency and duration of stops**"
    ...
    if primary_driver_freq:
        highest_stops = analysis_df.loc[analysis_df['stops'].idxmax()]; p_label = format_p(highest_stops['period'], analysis_level)
        example_insight = f"Example: <strong>{p_label}</strong> had the most stops (<strong>{int(highest_stops['stops'])}</strong>). Prioritize root cause of frequent events."
    elif primary_driver_dur:
        highest_mttr = analysis_df.loc[analysis_df['mttr'].idxmax()]; p_label = format_p(highest_mttr['period'], analysis_level)
        example_insight = f"Example: <strong>{p_label}</strong> had the longest downtimes (avg <strong>{highest_mttr['mttr']:.1f} min</strong>). Investigate prolonged stops."
    else: # Mixed
         highest_mttr = analysis_df.loc[analysis_df['mttr'].idxmax()]; p_label = format_p(highest_mttr['period'], analysis_level)
         example_insight = (f"Example: <strong>{p_label}</strong> experienced prolonged downtimes (avg <strong>{highest_mttr['mttr']:.1f} min</strong>), highlighting duration impact.")

    return f"<div style='line-height: 1.6;'><p>{corr_insight}</p><p>{example_insight}</p></div>"


# --- Excel Generation Function (Unchanged from previous correct version) ---
def generate_excel_report(all_runs_data, tolerance):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for run_id, data in all_runs_data.items():
            ...
            if not shot_time_col_dyn: missing_cols.append('SHOT TIME')
            ...
            table_formulas_ok = all(item is not None for item in [
                stop_col, stop_event_col, time_diff_col_dyn, cum_count_col_dyn, run_dur_col_dyn, bucket_col_dyn, shot_time_col_dyn
            ])
            ...
            # Use mode_ct value from Python calculation
            ...
            # Use lower/upper limits from Python calculation (static values) if available
            lower_limit_val = data.get('lower_limit', 0)
            upper_limit_val = data.get('upper_limit', 0)
            ws.write('F3', lower_limit_val if lower_limit_val is not None else f'=E2*(1-{tolerance})', secs_format)
            ws.write('G3', upper_limit_val if upper_limit_val is not None else f'=E2*(1+{tolerance})', secs_format)
            ...
            ws.write('F5', 'Tot Run Time', label_format); ws.write('G5', 'Tot Down Time', label_format)
             # Use Python calculated values for header times
            ws.write('F6', data['production_run_sec'] / 86400, time_format) # Wall clock
            ws.write('G6', data['tot_down_time_sec'] / 86400, time_format) # Sum stops
            # Calculate percentages based on the written times
            ...
            # Use Python calculated metrics (Static Values for consistency)
            ...
            if 'SHOT TIME' in df_run.columns:
                df_run['SHOT TIME'] = pd.to_datetime(df_run['SHOT TIME'], errors='coerce').dt.tz_localize(None) # Ensure datetime and remove tz
            ...
            for i, row_values in enumerate(df_run.to_numpy()):
                ...
                for c_idx, value in enumerate(row_values):
                    ...
                    # Skip columns that will be replaced by formulas
                    if col_name in ['CUMULATIVE COUNT', 'RUN DURATION', 'TIME BUCKET', 'TIME DIFF SEC']:
                        continue
                    # Write other pre-calculated values from Python DF
                    cell_format = data_format
                    if isinstance(value, pd.Timestamp):
                         if pd.notna(value):
                            ws.write_datetime(current_row_excel_idx - 1, c_idx, value, datetime_format)
                         else:
                            ws.write_string(current_row_excel_idx - 1, c_idx, '', data_format) # Write blank for NaT
                    elif isinstance(value, (bool, np.bool_)):
                        ws.write_number(current_row_excel_idx - 1, c_idx, int(value), data_format)
                    elif isinstance(value, (int, float, np.number)):
                         if col_name == 'ACTUAL CT' or col_name == 'adj_ct_sec': cell_format = secs_format # Format CT cols
            ...
            if table_formulas_ok:
                ...
                for i in range(len(df_run)):
                    ...
                    # Helper column formula (crucial for run duration sums)
                    if i == 0:
                        # First row uses its own TIME DIFF SEC if not a stop
                        helper_formula = f'=IF({stop_col}{row_num}=0, {time_diff_col_dyn}{row_num}, 0)'
                    else:
                        # Subsequent rows add previous helper value + current TIME DIFF SEC if not a stop start
                    ...
                    ws.write_formula(f'{helper_col_letter}{row_num}', helper_formula) # Write to hidden helper col

                    # TIME DIFF SEC formula (depends on SHOT TIME)
                    if i == 0:
                         # Write 0 for the first row as per previous logic fix
                         ws.write_number(row_num - 1, time_diff_col_idx, 0, secs_format)
                    else:
                        formula = f'=({shot_time_col_dyn}{row_num}-{shot_time_col_dyn}{prev_row})*86400'
                        ws.write_formula(row_num - 1, time_diff_col_idx, formula, secs_format)

                    # CUMULATIVE COUNT formula (depends on STOP EVENT and helper)
                    ...
                    # RUN DURATION formula (depends on STOP EVENT and helper)
                    ...
                    # TIME BUCKET formula (depends on STOP EVENT and helper)
            ...
            else: # Fallback if columns missing
...
def generate_run_based_excel_export(df_for_export, tolerance, downtime_gap_tolerance, run_interval_hours, tool_id_selection):
    """
    Prepares data for the Excel export using the main RunRateCalculator logic.
    """
    # 1. Base calculation using the main RunRateCalculator
    try:
        # Pass downtime_gap_tolerance here
    ...
    if df_processed.empty:
        st.error("Could not process data for Excel export. Check input data and required columns (e.g., SHOT TIME, ACTUAL CT).")
        return BytesIO().getvalue()

    # 2. Split into runs (using time_diff_sec, which is always present after _prepare_data)
    split_col = 'time_diff_sec'
    is_new_run = df_processed[split_col] > (run_interval_hours * 3600)
    # Ensure run_id starts from 1 if not already present
    df_processed['run_id'] = is_new_run.cumsum() + (0 if is_new_run.iloc[0] else 1)


    all_runs_data = {}
    desired_columns_base = [ # Columns expected from Calculator or original DF
        'SUPPLIER NAME', 'tool_id', 'SESSION ID', 'SHOT ID', 'shot_time',
        'APPROVED CT', 'ACTUAL CT',
        'time_diff_sec', 'stop_flag', 'stop_event', 'run_group'
        # 'adj_ct_sec' is useful internally but maybe not for final export table? Keep it simple.
    ]
    formula_columns = ['CUMULATIVE COUNT', 'RUN DURATION', 'TIME BUCKET']

    # 3. Loop through runs, recalculate with RunRateCalculator for run-specific metrics
    for run_id, df_run_raw in df_processed.groupby('run_id'):
        try:
            # Recalculate for THIS run using the SAME logic
            ...
            if not run_results or 'processed_df' not in run_results or run_results['processed_df'].empty:
                st.warning(f"Skipping empty or invalid Run ID {run_id} for Excel export.")
                continue

            # Add general run info
            ...
            # Select and rename columns for the final sheet
            columns_to_export = [col for col in desired_columns_base if col in export_df.columns]
            columns_to_export.extend(formula_columns) # Add formula cols at the end
            ...
            # Define final desired order and add missing columns as blank
            ...
            for col in final_desired_renamed:
             ...
             # Ensure final column order, keeping only existing ones after rename/add
            ...
            import traceback
            st.text(traceback.format_exc()) # More detailed error in console/log
    ...
    # 4. Generate the Excel file using the dedicated reporting function
    return generate_excel_report(all_runs_data, tolerance)


# --- Dashboard Rendering Function (render_dashboard) (Unchanged - Uses RunRateCalculator) ---
def render_dashboard(df_tool, tool_id_selection):
    ...
    def get_processed_data(df, interval_hours, tol, gap_tol):
        ...
        if not df_processed.empty:
            ...
            df_processed['run_id'] = is_new_run.cumsum() + (0 if is_new_run.iloc[0] else 1) # Ensure starting at 1
        return df_processed

    # Pass slider values for initial processing and run splitting
    ...
    if df_processed.empty:
        st.error(f"Could not process data for {tool_id_selection}. Check format/range."); st.stop()
    ...
    if not df_view.empty:
        ...
        if 'run_id' in df_view.columns:
            # Filter by min shots if in 'by Run' mode
            if 'by Run' in analysis_level:
                ...
                if runs_before > 0:
                     st.sidebar.metric("Runs Displayed", f"{runs_after} / {runs_before}", f"-{runs_before - runs_after} filtered", delta_color="off")

            # Create run labels for display AFTER filtering
            if not df_view.empty:
    ...
    if df_view.empty:
        ...
        # --- Calculate Final Metrics for Display/Export ---
        # Crucially, pass the correct analysis_mode and slider values
        ...
        if not results or 'processed_df' not in results or results['processed_df'].empty:
             st.error("Calculation failed for the selected period. Check data quality.")
             st.stop()

        summary_metrics = results # Use the results directly

        # Adjust header if combined runs
        if mode == 'by_run':
        ...
        # --- Rest of the Dashboard Display Code (Unchanged) ---
        # This part uses 'summary_metrics' and 'results' derived from the unified RunRateCalculator
        ...
        with st.container(border=True):
            ...
            if mode == 'by_run':
                ...
                with c2:
                    ...
                    with st.container(border=True): st.metric("Mode CT (sec)", f"{min_mc:.2f} – {max_mc:.2f}" if min_mc != max_mc else f"{min_mc:.2f}")
        ...
        # --- Detailed Analysis and Breakdowns (Unchanged) ---
        if detailed_view:
            st.markdown("---")
        ...
        # --- Plots (Unchanged) ---
        plot_shot_bar_chart(results['processed_df'], results.get('lower_limit'), results.get('upper_limit'), results.get('mode_ct'), time_agg=('hourly' if "Daily" in analysis_level else 'daily' if 'Weekly' in analysis_level else 'weekly'))
        ...
        st.markdown("---") # Separator before trend plots

        # --- Trend Plot Section (Unchanged) ---
        if "Daily" in analysis_level:
            st.header("Hourly Analysis")
            run_durations_period = results.get("run_durations", pd.DataFrame()); processed_period_df = results.get('processed_df', pd.DataFrame())
            stop_events_df = processed_period_df.loc[processed_period_df['stop_event']].copy(); complete_runs = pd.DataFrame()
            ...
            else: complete_runs = run_durations_period # Use all if no stops recorded
            c1,c2 = st.columns(2)
            with c1:
            ...
            hourly_summary = results.get('hourly_summary', pd.DataFrame())
            if hourly_summary is not None and not hourly_summary.empty:
                ...
                if detailed_view and hourly_summary['stops'].sum() > 0:
                     with st.expander("🤖 View MTTR/MTBF Correlation Analysis"): st.info("""**How this works:** Determines if stability is more affected by frequency (many small stops) or duration (few long stops)."""); analysis_df = hourly_summary.copy().rename(columns={'hour': 'period', 'stability_index': 'stability', 'stops': 'stops', 'mttr_min': 'mttr'}); st.markdown(generate_mttr_mtbf_analysis(analysis_df, analysis_level), unsafe_allow_html=True)
            ...
            summary_df = trend_summary_df # This is daily or weekly summary
            run_durations_period = results.get("run_durations", pd.DataFrame()); processed_period_df = results.get('processed_df', pd.DataFrame())
            stop_events_df = processed_period_df.loc[processed_period_df['stop_event']].copy(); complete_runs = pd.DataFrame()
            if not stop_events_df.empty:
                stop_events_df['terminated_run_group'] = stop_events_df['run_group'] - 1
            ...
            if not complete_runs.empty and summary_df is not None and not summary_df.empty and 'run_end_time' in complete_runs:
                ...
                all_units = summary_df[time_col].unique() # Ensure we have all days/weeks even if no runs ended then
                pivot_df = pivot_df.reindex(all_units, fill_value=0).sort_index()
            ...
            if summary_df is not None and not summary_df.empty:
                ...
                with st.expander("View MTTR/MTBF Data"): st.dataframe(summary_df)
                if detailed_view and summary_df['stops'].sum() > 0:
                     with st.expander("🤖 View MTTR/MTBF Correlation Analysis"): st.info("""**How this works:** Determines if stability is more affected by frequency or duration."""); analysis_df = summary_df.copy(); rename_map = {};
                     if 'date' in analysis_df.columns: rename_map = {'date': 'period', 'stability_index': 'stability', 'stops': 'stops', 'mttr_min': 'mttr'}
                     elif 'week' in analysis_df.columns: rename_map = {'week': 'period', 'stability_index': 'stability', 'stops': 'stops', 'mttr_min': 'mttr'}; analysis_df.rename(columns=rename_map, inplace=True); st.markdown(generate_mttr_mtbf_analysis(analysis_df, analysis_level), unsafe_allow_html=True)

        elif "by Run" in analysis_level:
            st.header(f"Run-Based Analysis")
            run_summary_df = trend_summary_df # This is the run summary df
            run_durations_period = results.get("run_durations", pd.DataFrame()); processed_period_df = results.get('processed_df', pd.DataFrame())
            stop_events_df = processed_period_df.loc[processed_period_df['stop_event']].copy(); complete_runs = pd.DataFrame()
            if not stop_events_df.empty:
                stop_events_df['terminated_run_group'] = stop_events_df['run_group'] - 1
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.subheader("Total Bucket Analysis (Across Runs)")
                if not complete_runs.empty and "time_bucket" in complete_runs.columns:
                    b_counts = complete_runs["time_bucket"].value_counts().reindex(results["bucket_labels"], fill_value=0)
                    fig_b = px.bar(b_counts, title="Total Time Bucket Analysis", labels={"index": "Duration(min)", "value": "Occurrences"}, text_auto=True, color=b_counts.index, color_discrete_map=results["bucket_color_map"]).update_layout(legend_title_text='Duration')
                    st.plotly_chart(fig_b, use_container_width=True)
            ...
            if not complete_runs.empty and run_summary_df is not None and not run_summary_df.empty and 'run_end_time' in complete_runs:
                ...
                complete_runs['run_label'] = complete_runs['run_group'].map(run_group_to_label_map)
                 # Filter complete_runs to only include runs present in the filtered run_summary_df
                ...
                if not complete_runs_filtered.empty:
                    pivot_df = pd.crosstab(index=complete_runs_filtered['run_label'], columns=complete_runs_filtered['time_bucket'].astype('category').cat.set_categories(results["bucket_labels"]))
                    pivot_df = pivot_df.reindex(valid_run_labels, fill_value=0) # Reindex based on filtered runs
                    fig_bucket_trend = make_subplots(specs=[[{"secondary_y": True}]])
                    for col in pivot_df.columns: fig_bucket_trend.add_trace(go.Bar(name=col, x=pivot_df.index, y=pivot_df[col], marker_color=results["bucket_color_map"].get(col)), secondary_y=False)
                    # Align shot counts with the filtered runs
                    shots_df = run_summary_df.set_index('RUN ID').reindex(valid_run_labels)
                    fig_bucket_trend.add_trace(go.Scatter(name='Total Shots', x=shots_df.index, y=shots_df['Total Shots'], mode='lines+markers+text', text=shots_df['Total Shots'], textposition='top center', line=dict(color='blue')), secondary_y=True)
                    ...
                    with st.expander("View Bucket Trend Data"): st.dataframe(pivot_df)
                    if detailed_view:
                        with st.expander("🤖 View Bucket Trend Analysis"): st.markdown(generate_bucket_analysis(complete_runs_filtered, results["bucket_labels"]), unsafe_allow_html=True)
                else: st.info("No completed runs found for the filtered runs.")
            ...
            if run_summary_df is not None and not run_summary_df.empty:
                ...
                with st.expander("View MTTR/MTBF Data"): st.dataframe(run_summary_df)
                if detailed_view and run_summary_df['STOPS'].sum() > 0:
                     with st.expander("🤖 View MTTR/MTBF Correlation Analysis"): st.info("""**How this works:** Determines if stability is more affected by frequency or duration."""); analysis_df = run_summary_df.copy().rename(columns={'RUN ID': 'period', 'STABILITY %': 'stability', 'STOPS': 'stops', 'MTTR (min)': 'mttr'}); st.markdown(generate_mttr_mtbf_analysis(analysis_df, analysis_level), unsafe_allow_html=True)


# --- Risk Tower Functions (Unchanged - Uses RunRateCalculator) ---
@st.cache_data(show_spinner="Analyzing tool performance for Risk Tower...")
def calculate_risk_scores(df_all_tools):
    id_col = "tool_id"; initial_metrics = []
    default_tol, default_gap = 0.05, 2.0 # Use standard defaults for risk tower

    for tool_id, df_tool in df_all_tools.groupby(id_col):
        if df_tool.empty or len(df_tool) < 10: continue
        try:
             calc_prepare = RunRateCalculator(df_tool, default_tol, default_gap)
             df_prepared = calc_prepare.results.get("processed_df")
             if df_prepared is None or df_prepared.empty: continue

             end_date = df_prepared['shot_time'].max(); start_date = end_date - timedelta(weeks=4)
             ...
             calc = RunRateCalculator(df_period.copy(), default_tol, default_gap)
             res = calc.results
             if not res: continue # Skip if calculation failed

             df_period['week'] = df_period['shot_time'].dt.isocalendar().week
             weekly_stabilities = [RunRateCalculator(df_week, default_tol, default_gap).results.get('stability_index', 0) for _, df_week in df_period.groupby('week') if not df_week.empty]
             trend = "Stable"
             ...
             st.warning(f"Could not calculate risk score for {tool_id}: {e}")

    if not initial_metrics: return pd.DataFrame()
    metrics_df = pd.DataFrame(initial_metrics)
    overall_mttr_mean = metrics_df['MTTR'].mean(); overall_mtbf_mean = metrics_df['MTBF'].mean()
    final_risk_data = []
    for _, row in metrics_df.iterrows():
        risk_score = row['Stability']; details = f"Overall stability is {row['Stability']:.1f}%."
        if row['Trend'] == "Declining": risk_score -= 20; primary_factor = "Declining Trend"; details = "Stability shows a consistent downward trend."
        elif row['Stability'] < 70 and row['MTTR'] > (overall_mttr_mean * 1.2): primary_factor = "High MTTR"; details = f"Avg stop duration (MTTR) {row['MTTR']:.1f} min is key concern."
        elif row['Stability'] < 70 and row['MTBF'] < (overall_mtbf_mean * 0.8): primary_factor = "Frequent Stops"; details = f"Frequent stops (MTBF {row['MTBF']:.1f} min) impacting stability."
        else: primary_factor = "Low Stability" # Default if declining/MTTR/MTBF aren't primary drivers
...
def load_all_data(files):
    ...
    for file in files:
        try:
            ...
            # Time parsing happens in calculator now, just check for presence
            if "tool_id" in df.columns and (("SHOT TIME" in df.columns) or ({"YEAR", "MONTH", "DAY", "TIME"}.issubset(df.columns))):
                df_list.append(df)
            elif "tool_id" not in df.columns:
                 st.warning(f"Skipping file {file.name}: Missing 'TOOLING ID' or 'EQUIPMENT CODE'.")
            else: # Missing time column
                 st.warning(f"Skipping file {file.name}: Missing 'SHOT TIME' or YEAR/MONTH/DAY/TIME columns.")
...
if id_col not in df_all_tools.columns:
    st.error(f"No valid data loaded. Ensure files contain '{id_col}' (or variations) and time columns.")
    st.stop()

df_all_tools.dropna(subset=[id_col], inplace=True) # Drop rows where tool_id is missing after loading
...
if not unique_tool_ids:
     st.error("No tools found in the loaded data after cleaning.")
     st.stop()
...
if dashboard_tool_id_selection == "All Tools (Risk Tower)":
    # Default to first actual tool for dashboard view if "All" is selected
...
with tab2:
    if not df_for_dashboard.empty:
        ...
        st.info("Select a specific Tool ID from the sidebar to view its dashboard, or ensure the default tool has data.")