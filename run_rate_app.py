...
def generate_llm_prompt(main_results, trend_df, analysis_level):
    ...
    if trend_df is not None and not trend_df.empty:
        ...
        trend_cols = [col for col in ['date', 'week', 'RUN ID', 'stability_index', 'STABILITY %', 'mttr_min', 'MTTR (min)', 'mtbf_min', 'MTBF (min)', 'stops', 'STOPS'] if col in trend_df.columns]
...
if df_view.empty:
    ...
    if analysis_level == "Weekly":
        ...
        trend_summary_df = results.get('hourly_summary')
        if not trend_summary_df.empty:
            trend_summary_df.rename(columns={'hour': 'Hour'}, inplace=True)
    ...
    if analysis_level == "Weekly":
        ...
        run_summary_df = trend_summary_df # Already calculated
    ...
    if analysis_level == "Daily":
        ...
        summary_df = trend_summary_df # Already calculated
