    ...
    if st.session_state.show_llm_analysis:
        ...
        if trend_summary_df is not None and not trend_summary_df.empty and trend_summary_df.shape[0] > 1:
            ...
            if period_col and stability_col in trend_summary_df.columns:
                ...
                stops_during_lowest = int(min_stability_row[stops_col])
                ...
                trend_analysis_text = f"The stability trend shows some volatility. The lowest performance was recorded during <strong>{lowest_stability_period}</strong> with a stability of <strong>{lowest_stability_value:.1f}%</strong>. This period experienced <strong>{stops_during_lowest} stop events</strong>, highlighting the direct impact of stoppages on performance."

        # --- Improved Recommendation Logic ---
        recommendation = "Mean Time Between Failures (MTBF)"
        recommendation_value = f"{mtbf:.1f}"
        recommendation_text = "investigating the root causes of frequent stops to improve uptime."
        
        # Switch to MTTR if its impact is proportionally high
        if mtbf > 0 and mttr > 0:
            repair_impact_ratio = mttr / mtbf
            if repair_impact_ratio > 0.1: # If avg repair time is >10% of avg uptime
                recommendation = "Mean Time To Repair (MTTR)"
                recommendation_value = f"{mttr:.1f}"
                recommendation_text = "streamlining the repair process to reduce downtime."
        elif mttr > 0: # Handle case where MTBF might be 0 or NaN
             recommendation = "Mean Time To Repair (MTTR)"
             recommendation_value = f"{mttr:.1f}"
             recommendation_text = "addressing the root cause of any recorded stops, as the time to repair is significant."
        ...
        with st.expander("View Run Breakdown Table", expanded=False):
            if run_summary_df is not None and not run_summary_df.empty:
                ...
                d_df["Downtime (& %)"] = d_df.apply(
                    lambda row: f"{format_duration(row['downtime_sec'])} ({row['total_runtime_sec'] / row['total_runtime_sec'] * 100:.1f}%)" if row['total_runtime_sec'] > 0 else "0m (0.0%)",
                    axis=1
                )
