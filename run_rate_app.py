...
class RunRateCalculator:
    ...
    def _calculate_all_metrics(self) -> dict:
        ...
        # --- CORRECTED & SIMPLIFIED Stop Detection Logic ---
        # A stop is now defined as any shot where the actual time elapsed since the previous shot
        # ('time_diff_sec') falls outside the acceptable tolerance band based on the Mode CT.
        # This single condition effectively captures both abnormal cycle times and downtime gaps.
        df["stop_flag"] = np.where(
            (df["time_diff_sec"] < lower_limit) | (df["time_diff_sec"] > upper_limit),
            1,
            0
        )
...
def render_dashboard(df_tool, tool_id_selection):
    ...
    def get_processed_data(df, interval_hours):
        ...
        if not df_processed.empty:
            ...
            # --- BUG FIX: Corrected column name from 'ct_diff_sec' to 'time_diff_sec' ---
