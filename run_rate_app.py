...
class RunRateCalculator:
    ...
    def _calculate_all_metrics(self) -> dict:
        ...
        # --- Universal "by Run" Mode Calculation ---
        if 'run_id' in df.columns and df['run_id'].nunique() > 0:
            ...
            # Fallback to aggregate mode if only one run or no run_id
    ...
    if analysis_level in ["Weekly", "Monthly"]:
        ...
        with st.expander("View Run Breakdown Table", expanded=False):
            if not run_summary_df.empty:
                ...
                d_df.rename(columns={'run_label': 'Run ID', 'stability_index': 'Stability (%)', 'mttr_min': 'MTTR (min)', 
                                     'mtbf_min': 'MTBF (min)', 'stops': 'Stops', 'mode_ct': 'Mode CT', 
                                     'lower_limit':'LL', 'upper_limit':'UL'}, inplace=True)
                st.dataframe(d_df[['Run ID', 'Stability (%)', 'MTTR (min)', 'MTBF (min)', 'Stops', 'Mode CT', 'LL', 'UL']].style.format(
                    {'Stability (%)': '{:.1f}', 'MTTR (min)': '{:.1f}', 'MTBF (min)': '{:.1f}', 'Mode CT': '{:.2f}', 'LL': '{:.2f}', 'UL': '{:.2f}'}), use_container_width=True)
